...
源图：豆包生产物料/app图标/用户选中_原图.png
（1024x1024，圆角绿色渐变 + 白色八角罗盘星 + 橙色圆心 + 虚线路线 + 路牌 + 远山；
 2026-09-20 由旧版「紫色渐变 + 折叠地图 + 橙色定位针」更换而来，
 旧图备份在工作区的 .dsh-icon-work/backup/ 下）
该图四角是纯白（不是透明），所以这里统一做两件事：
  1. 按原设计的圆角半径重新裁一个 alpha 遮罩，把白角变成透明（避免在深色壁纸上露出白方块）
  2. 另外裁一个正圆版本给 ic_launcher_round

配套改动：drawable/ic_launcher_bg.xml 与 drawable/splash_bg.xml 的渐变已同步换成新图标的实测取色
（#479C87 → #66B29A），否则自适应图标遮罩视差处会露出旧紫色。
